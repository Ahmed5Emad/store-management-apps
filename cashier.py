...
class ReceiptDialog(QDialog):
    def __init__(self, parent, total, change):
        super().__init__(parent)
        self.setWindowTitle("Receipt")
        self.setWindowModality(Qt.ApplicationModal)  # Make it the modal dialog
        self.resize(300, 300)
...
------------------------------------
""".format(date, time)
        for product_id, quantity in parent.cart.items():  # Use parent.cart
            receipt_text += f"{parent.products[product_id]['name']} x {quantity} - EG{parent.products[product_id]['price'] * quantity:.2f}\n"
        receipt_text += """
------------------------------------
Total: EG {total:.2f}
Cash: EG {payment_amount:.2f}
Change: EG {change:.2f}
------------------------------------
""".format(total=total, payment_amount=total + change, change=change)

        # Display receipt in QTextEdit
        self.receipt_area = QTextEdit(self)
        self.receipt_area.setReadOnly(True)
        self.receipt_area.setFont(QFont("Arial", 10))
        self.receipt_area.setText(receipt_text)
        layout.addWidget(self.receipt_area)
        ...
        self.timer.start(6000)  # 6000 milliseconds = 6 seconds
...
class CashierApp(QWidget):
    ...
    def show_receipt(self, total, change):
        # Create a separate window for the receipt
        receipt_dialog = ReceiptDialog(self, total, change)
        receipt_dialog.exec_()  # Show the receipt window
